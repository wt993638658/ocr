...
def get_det_boxes(image,display = True, expand = True):
    ...
    image_r = image.copy()
    image_c = image.copy()
    h, w = image.shape[:2]#放缩后的h,w
    # image = image.astype(np.float32) - config.IMAGE_MEAN#数据减去均值
    image = image - config.IMAGE_MEAN#数据减去均值
    image = torch.from_numpy(image.transpose(2, 0, 1)).unsqueeze(0).float()#重排列维度+在第0维添加一个维度
    with torch.no_grad():
        image = image.to(device)
        cls, regr = model(image)
        cls_prob = F.softmax(cls, dim=-1).cpu().numpy()#对最后一个维度归一化，是前景还是背景
        regr = regr.cpu().numpy()
        print(h,w)
        anchor = gen_anchor((int(h / 16), int(w / 16)), 16)#把一个图片分成很多个16*16的区域，每个区域定10个格
        # print(anchor.shape)
        # print(regr.shape)
        bbox = bbox_transfor_inv(anchor, regr)
        # print(bbox.shape)
        bbox = clip_box(bbox, [h, w])#边界修正
        fg = np.where(cls_prob[0, :, 1] > prob_thresh)[0]
        select_anchor = bbox[fg, :]
        select_score = cls_prob[0, fg, 1]
        select_anchor = select_anchor.astype(np.int32)
        keep_index = filter_bbox(select_anchor, 16)#过滤掉小于16像素的框
