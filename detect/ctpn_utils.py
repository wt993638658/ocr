...
def gen_anchor(featuresize, scale):
    ...
    """
    # print(featuresize,scale)
    heights = [11, 16, 23, 33, 48, 68, 97, 139, 198, 283]
    widths = [16, 16, 16, 16, 16, 16, 16, 16, 16, 16]

    # gen k=9 anchor size (h,w)
    heights = np.array(heights).reshape(len(heights), 1)#从列表变成(10,1)的nd数组
...
def bbox_transfor_inv(anchor, regr):
    """
        return predict bbox
    """
    # print(anchor.shape,regr.shape)
    Cya = (anchor[:, 1] + anchor[:, 3]) * 0.5#y的中心
    # print(Cya.shape)
    ha = anchor[:, 3] - anchor[:, 1] + 1#y的距离
