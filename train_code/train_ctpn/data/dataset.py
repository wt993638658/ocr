...
class ICDARDataset(Dataset):
    ...
    def parse_gtfile(self,gt_path,rescale_fac = 1.0):
        ...
        return self.box_transfer_v2(coor_lists,rescale_fac)#用坐标求出所有的小框
    ...
    def __getitem__(self, idx):
        img_name = self.img_names[idx]#获取图像文件名
        # print(img_name)
        ...
        # print(rescale_fac)
        if rescale_fac>1.0:#图像大于1600就缩小
            h = int(h/rescale_fac)
            w = int(w/rescale_fac)
            img = cv2.resize(img,(w,h))

        gt_path = os.path.join(self.labelsdir, 'gt_'+img_name.split('.')[0]+'.txt')#获取
        # print(gt_path)
        gtbox = self.parse_gtfile(gt_path,rescale_fac)#通过四个标签，求出所有小框
        ...
        cls = torch.from_numpy(cls).float()
        regr = torch.from_numpy(regr).float()
        print(m_img.shape,cls.shape,regr.shape)
        return m_img, cls, regr
