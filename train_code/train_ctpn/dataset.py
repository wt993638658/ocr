...
class ICDARDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # clip image
        if np.random.randint(2) == 1:##随机左右翻转
            img = img[:, ::-1, :]
            newx1 = w - gtbox[:, 2] - 1
            newx2 = w - gtbox[:, 0] - 1
            gtbox[:, 0] = newx1
...
def parse_gtfile(gt_path,rescale_fac = 1.0):
    coor_lists = list()
    with open(gt_path,encoding='UTF-8') as f:
        content = f.readlines()
        for line in content:
            ...
            if len(coor_list)==8:
                coor_lists.append(coor_list)
    return box_transfer_v2(coor_lists,rescale_fac)
#
...
if __name__ == '__main__':
    gtbox = parse_gtfile("./train_data/train_label\gt_img_7001.txt")
    # xmin = 15
    # xmax = 95
    # for i in range(xmin//16+1,xmax//16+1):
    #     print(16*i-0.5)